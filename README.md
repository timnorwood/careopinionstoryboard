...
5. Open the Example monthly update script. Change the Getfrom date to at least two years prior. It could take 1-2 hours to get two years data but 20 months are needed for some charts. In future updates Getfrom can be three months prior so it runs faster. Check the other parameters (dates, labels, NACorg filters etc) and amend these as required. Organisation and service codes used by Care Opinion can be found in the url of searches (e.g. the url for a search of stories about NHS Grampian is https://www.careopinion.org.uk/opinions?nacs=SN9)
