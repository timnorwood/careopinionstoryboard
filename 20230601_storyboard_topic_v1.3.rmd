...
tplot = allFrame %>%
  mutate(timerange = case_when(
    Date.y >= as.Date("2023-04-01") & Date.y <= as.Date("2024-03-31") ~ "ctl",
    Date.y >= as.Date("2024-04-01") & Date.y <= as.Date("2025-03-31") ~ "trt")) %>% 
...
plot_ly(tplot, x = ~tagName, type = 'bar', y = ~round(pct_ctl,2), name = '2023/24', marker = list(color = 'rgba(173, 65, 87,0.2)')) %>%
  add_trace(y = ~round(pct_trt,2),  name = '2024/25', marker = list(color = 'rgb(173, 65, 87)')) %>%
  layout(xaxis = list(title = '', tickangle = -45),
...
plot_ly(tplot, x = ~tagName, type = 'bar', y = ~round(pct_ctl,2), name = '2023/24', marker = list(color = 'rgba(125, 145, 105,0.3)')) %>%
  add_trace(y = ~round(pct_trt,2),  name = '2024/25', marker = list(color = 'rgb(125, 145, 105)')) %>%
  layout(xaxis = list(title = '', tickangle = -45),
