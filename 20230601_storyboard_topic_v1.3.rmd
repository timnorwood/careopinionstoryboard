...
#Send any files folders to the temp folder so they don't remain in output folder (need when running on local)
...
tplot = allFrame %>%
  mutate(timerange = case_when(
    Date.y >= as.Date("2024-04-01") & Date.y <= as.Date("2025-03-31") ~ "ctl",
    Date.y >= as.Date("2025-04-01") & Date.y <= as.Date("2026-03-31") ~ "trt")) %>% 
...
plot_ly(tplot, x = ~tagName, type = 'bar', y = ~round(pct_ctl,2), name = '2024/25', marker = list(color = 'rgba(173, 65, 87,0.2)')) %>%
  add_trace(y = ~round(pct_trt,2),  name = '2025/26', marker = list(color = 'rgb(173, 65, 87)')) %>%
  layout(xaxis = list(title = '', tickangle = -45),
...
plot_ly(tplot, x = ~tagName, type = 'bar', y = ~round(pct_ctl,2), name = '2024/25', marker = list(color = 'rgba(125, 145, 105,0.3)')) %>%
  add_trace(y = ~round(pct_trt,2),  name = '2025/26', marker = list(color = 'rgb(125, 145, 105)')) %>%
  layout(xaxis = list(title = '', tickangle = -45),
...
If you would like to know more please contact the Healthcare Improvement Scotland Data, Measurement and Business Intelligence team at [hcis.DMBI-TEAM\@nhs.scot](mailto:hcis.DMBI-TEAM@nhs.scot){.email}.
